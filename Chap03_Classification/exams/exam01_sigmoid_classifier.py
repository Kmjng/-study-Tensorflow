...
# csv file load
bmi = pd.read_csv('C:/ITWILL/7_Tensorflow/Tensorflow/data/bmi.csv')
print(bmi.info())
...
'''
data.reshape([-1,1])
원래 배열을 행이 여러 개, 열이 1개인 2차원 배열로 변환
[[0., 1.],
'''
...
# 7. 반복학습 : 1,000번, 100 step 단위로 loss 출력 
for step in range(1000):
    opt.minimize(loss = loss_fn, var_list = [w,b])
    # 10배수 단위 출력 
    if (step+1) % 10 == 0 :
        print('step =', (step+1), ", loss val = ", loss_fn().numpy())
# step = 1000 , loss val =  0.27888975

# 8. 최적화된 model 테스트
y_pred = sig_fn(X) # 모델링한 데이터셋 예측값
y_pred.numpy()
'''
array([[0.23418485, 0.6719014 ],
       [0.2921064 , 0.84130913],
       [0.03818528, 0.9562296 ],
'''

y_pred = tf.cast(y_pred.numpy()> 0.5, dtype=tf.float32)
y_pred
'''
array([[0., 1.],
       [0., 1.],
